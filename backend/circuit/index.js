...
const createRedisBackend = (client, prefix = DEFAULT_PREFIX) => ({
  ...
  async recordFailure(now, windowMs, threshold, cooldownMs) {
    ...
    if (count >= threshold) {
      ...
      logger.warn('OpenAI circuit opened (redis)', { openedCount: await client.get(`${prefix}:openedCount`) });
    }
...
function init() {
  ...
  if (url) {
    try {
      ...
      logger.warn('Failed to initialize Redis client, falling back to in-memory circuit', { err: e.message });
      backend = inMemory;
    }
  } else {
    backend = inMemory;
...
module.exports = {
  ...
  recordFailure: async (now, windowMs, threshold, cooldownMs) => backend.recordFailure(now, windowMs, threshold, cooldownMs),
